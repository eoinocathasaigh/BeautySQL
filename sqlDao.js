    ...
    return new Promise((resolve, reject) => {
        Promise.all([
            ...
            pool.query(`SELECT 
                        ...
                        COUNT(si.user_id) AS current_participants
                        FROM sampling_campaigns sc
                        JOIN product p ON sc.product_id = p.id
                        JOIN brand b ON p.brand_id = b.id
                        LEFT JOIN sampler_interactions si ON sc.id = si.campaign_ref
                        WHERE sc.campaign_ref = ?
                        GROUP BY sc.id;`, [campaignRef]),
            ...
            pool.query(`SELECT 
                        ...
                        p.instagram_handle,
                        p.instagram_followers,
                        si.status AS campaign_status,
                        si.video_content,
                        r.rating AS review_rating,
                        r.title AS review_title
                    FROM sampling_campaigns sc
                    JOIN sampler_interactions si ON sc.id = si.campaign_ref
                    JOIN user u ON si.user_id = u.id
                    JOIN profile p ON u.id = p.user_id
                    LEFT JOIN review r ON u.id = r.user_id
                        AND r.product_variant_id IN (
                            SELECT id FROM product_variant 
                            WHERE product_id = sc.product_id
                        )
                    WHERE sc.campaign_ref = ?;`, [campaignRef])
        ])
            .then(([campaignData, participantsData]) => {
                resolve({
...
module.exports = { getMembers, memberDetails, getParticipants, getCampaigns, getCamps, getCampaignDetails, updateMember };